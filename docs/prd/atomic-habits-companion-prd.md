...
| Chunk | Scope | Outcome |
|-------|--------|--------|
| **Chunk 1** | Identity + focus onboarding | One identity (default), one habit with 2-min version, lightweight 4 Laws, first completion in-flow, then optional add another identity. Onboarding ends with a win. |
...
- Identities page: scoreboard only (read-only). Per identity: statement, "This week: N votes", trend (↑/↓ vs last week or "—"), Momentum bar; "Reinforced by" up to 3 habits (read-only, "View all" if more); "Undermined by" up to 2 blockers (read-only) or empty state. CTAs: "+ Add reinforcing habit" → Identity detail with ?add=1; "Add a blocking habit" / "View & fix blockers →" → Identity detail with ?blockers=1; "View details" → Identity detail (/dashboard/identities/[id]). Identity detail page (D52): scoreboard, full habits list for that identity, blockers section (add/edit habit to break on same page). Identity CRUD on Identities list; habit and blocker create/edit on Identity detail and Habits page (Habits remains single source of truth for habit form; identity detail has blockers inline).
...
- Blockers section on Habits page: add/edit one habit to break per identity; user can pick from existing habits ("Add as blocker") or add by name, then complete 4-laws break form. When picking from list, only habits that are not linked as reinforcing any identity (identity_id is null) are shown; habits linked to any identity are excluded. habit_id stored when picked from list; shown when mode=fix and identity set, or via "Add a blocking habit" / "View & fix blockers" from Identities
...
- Per identity: user can add/edit one "habit to break" on the Habits page (blockers section when mode=fix or via "Add a blocking habit" / "View & fix blockers" from Identities): pick from existing unlinked habits only (Add as blocker) or enter name, then 4-laws (break) design; habits_to_break stores name, design_break, and optional habit_id when picked; Identities page shows read-only list (up to 2); CTA is "Add a blocking habit" when no blockers, "View & fix blockers →" when blockers exist
...
**Implementation notes (built 2026-02-22):** Facelift is the **default Review tab**: `/dashboard/review` shows the two-column weekly review (current week) directly. Shared component `WeeklyReviewContent` used on both Review and Write. Two-column layout: left = habit scoreboard (one card per habit with M–S day letters, name, identity, streak + completed/scheduled, link to habit detail) + weekly reflection card (James Clear quote, 4-prompt 2×2 grid, Save); right = Wins & warnings + Atomic Habits insight + Identity votes. Week prev/next on both pages. **Custom time range:** link "Review a different week" on Review tab and in "Other tools" goes to `/dashboard/review/write` (same UI, week from URL `?date=YYYY-MM-DD`). Write page shows "← Review" back link and title "Weekly Review". Below the facelift on Review tab, compact "Other tools" section: Review a different week, Past reviews, Rate & fix, Map your day, I'm stuck — reset, Monthly reflection. API `/api/reviews/summary` returns `week_completion`, `scheduled_days`, `completed_count` per habit. Hub list removed; GraduationPrompt retained. No schema changes.
...
| D40 | Identity ↔ habit to break | habits_to_break table, one per identity | Every identity can have one contradicting/negating habit to break. User enters the habit name and a "how to break it" design using the 4 laws inversion (invisible, unattractive, difficult, unsatisfying), 3 sub-points each. Stored in habits_to_break.design_break. UI: add/edit on Habits page only (blockers section when identity&mode=fix); Identities page shows read-only "Undermined by" and CTA: "Add a blocking habit" when no blockers, "View & fix blockers →" when blockers exist (see D51). | 2026-02-20 |
| D46 | Identities as scoreboard; Identity detail; Habits single source of truth | Identities list + identity detail page; post-create prompt; deep-links | Identities list is a scoreboard per identity with CTAs. Identity detail at /dashboard/identities/[id] (D52) shows scoreboard, full habits list, and blockers section on one page. Post-create identity prompt: "Create habit now" → /dashboard/identities/[id]?add=1; "Skip for now" closes. Habits page remains where habit create/edit form lives; identity detail links to it for "Add reinforcing habit" and shows blockers inline. | 2026-02-20 |
...
| D48 | Pick existing habit as blocker | habits_to_break.habit_id; Blockers UI | When adding a habit to break (mode=fix), user can pick from existing habits list ("Add as blocker") so name is prefilled and optional habit_id is stored, or add by name only. Only habits not linked as reinforcing any identity (identity_id is null) are shown in the pick list (see D51). habits_to_break has optional habit_id FK to habits.id (ON DELETE SET NULL). | 2026-02-20 |
